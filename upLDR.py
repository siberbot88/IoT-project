...
while True:
    try:
        ...
        # Kontrol LED suhu
        led.value(1 if suhu > 30 else 0)
        
        # Kontrol LED merah berdasarkan LDR
        led_red.value(1 if ldr_value > LDR_DARK_THRESHOLD else 0)

        # Kirim ke Ubidots
        payload = ujson.dumps({
            "temperature": ldr_value,
            "humidity": kelembaban,
            "led": led.value(),
            "motion": gerakan,
            "ldr": ldr_value
        ...
        print(f"💡 LED Merah: {'ON' if led_red.value() else 'OFF'}")
